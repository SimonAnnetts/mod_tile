...
struct projectionconfig * get_projection(const char * srs) {
    ...
    if (strstr(srs,"+proj=merc +a=6378137 +b=6378137") != NULL) {
        ...
        prj->aspect_y = 1;
                ...
                strcmp(srs, "+proj=tmerc +lat_0=49 +lon_0=-2 +k=0.999601 +x_0=400000 +y_0=-100000 +ellps=airy  +units=m +towgs84=446.448,-125.157,542.060,0.1502,0.2470,0.8421,-20.4894 +units=m +nodefs") == 0) {
        syslog(LOG_DEBUG, "Using bng EPSG:27700 projection settings");
        prj = (struct projectionconfig *)malloc(sizeof(struct projectionconfig));
        prj->bound_x0 = -350000;
        prj->bound_y0 = -100000;
        prj->bound_x1 = 1050000;
        prj->bound_y1 = 1300000;
        prj->aspect_x = 1;
        prj->aspect_y = 1;
    } else {
        syslog(LOG_WARNING, "Unknown projection string '%s', using web mercator instead.", srs);
        prj = (struct projectionconfig *)malloc(sizeof(struct projectionconfig));
        prj->bound_x0 = -20037508.3428;
        prj->bound_x1 =  20037508.3428;
        prj->bound_y0 = -20037508.3428;
